...
import { computeCoreCycles, Core, CoreCycles, to as beatenPathTo } from '../../../../src/indexForTest'
...
describe('core cycles', () => {
    ...
    describe('core intervals', () => {
        it('when core is 2', () => {
            ...
            const superp: Fraction = to.Fraction([ to.Numerator(2), to.Denominator(1) ])
            const subp: Fraction = to.Fraction([ to.Numerator(2), to.Denominator(3) ])

            const expectedIntervals: Cycle<Fraction> = to.Cycle([
                subp,
                superp,
                subp,
                subp,
        ...
        it('when core is 3', () => {
            ...
            const superp: Fraction = to.Fraction([ to.Numerator(3), to.Denominator(2) ])
            const subp: Fraction = to.Fraction([ to.Numerator(3), to.Denominator(4) ])
        ...
        it('when core is 4', () => {
            ...
            const superp: Fraction = to.Fraction([ to.Numerator(4), to.Denominator(3) ])
            const subp: Fraction = to.Fraction([ to.Numerator(4), to.Denominator(5) ])
        ...
        it('when core is 5', () => {
            ...
            const superp: Fraction = to.Fraction([ to.Numerator(5), to.Denominator(4) ])
            const subp: Fraction = to.Fraction([ to.Numerator(5), to.Denominator(6) ])
    ...
    describe('core durations', () => {
        for (let core: Core = beatenPathTo.Core(2); core <= beatenPathTo.Core(7); core = apply.Translation(core, to.Translation(beatenPathTo.Core(1)))) {
                ...
                it('each successive duration is equal to the previous duration multiplied by the previous interval', () => {
                    for (
                        ...
                        const previousDuration: Scalar = apply.Ordinal(
                            coreDurations,
                            apply.Translation(index, PREVIOUS) as Ordinal<Scalar>,
                        )
                        const previousIntervalAsScalar: Scalar<Scalar> = to.Scalar(to.Scalar(from.Fraction(apply.Ordinal(
                            coreIntervals,
                            apply.Translation(index, PREVIOUS) as Ordinal<Fraction>,
                        ))))
                        expect(apply.Ordinal(coreDurations, index as Ordinal<Scalar>))
        ...
        describe(
            ...
            () => {
                it(`for core 2, in monzo form it's | 84 -53 >, AKA the reciprocal of Mercator's comma`, () => {
                    ...
                    const finalIntervalAsScalar: Scalar<Scalar> = to.Scalar(to.Scalar(from.Fraction(finalElement(coreIntervals))))
        ...
        describe('in terms of subps and superps', () => {
            it('for core 2', () => {
                ...
                const totalIntervals: Power = to.Power(from.Cardinal(totalElements(coreIntervals)))
                expect(nextDurationWouldHaveBeen)
                    .toBeCloseToTyped(quotient(
                        apply.Power(2, totalIntervals),
                        product(
                            apply.Power(3, totalUpIntervals),
                            apply.Power(1, totalDownIntervals),
                        ),
                    ))
                expect(to.Power(sum(totalUpIntervals, totalDownIntervals)))
                    .toBe(totalIntervals)
            })
            ...
            it('for core 3', () => {
                ...
                expect(nextDurationWouldHaveBeen)
                    .toBeCloseToTyped(quotient(
                        apply.Power(3, totalIntervals),
                        product(
                            apply.Power(4, totalUpIntervals),
                            apply.Power(2, totalDownIntervals),
                        ),
                    ))
                expect(to.Power(sum(totalUpIntervals, totalDownIntervals)))
                    .toBe(totalIntervals)
            ...
            it('for core 4', () => {
                ...
                expect(nextDurationWouldHaveBeen)
                    .toBeCloseToTyped(quotient(
                        apply.Power(4, totalIntervals),
                        product(
                            apply.Power(5, totalUpIntervals),
                            apply.Power(3, totalDownIntervals),
                        ),
                    ))
                expect(to.Power(sum(totalUpIntervals, totalDownIntervals)))
                    .toBe(totalIntervals)
            ...
            it('for core 5', () => {
                ...
                expect(nextDurationWouldHaveBeen)
                    .toBeCloseToTyped(quotient(
                        apply.Power(5, totalIntervals),
                        product(
                            apply.Power(6, totalUpIntervals),
                            apply.Power(4, totalDownIntervals),
                        ),
                    ))
                expect(to.Power(sum(totalUpIntervals, totalDownIntervals)))
                    .toBe(totalIntervals)
    ...
    (because they are cycles, so there is the same number of intervals between things as there are things (durations)`,
        () => {
            it('for core 2, they are both 84', () => {
                ...
                coreIntervals = coreCycles.coreIntervals
                coreDurations = coreCycles.coreDurations
                expect(totalElements(coreIntervals))
                    .toBe(to.Cardinal(84))
                expect(totalElements(coreDurations))
                    .toBe(to.Cardinal(84))
            })
